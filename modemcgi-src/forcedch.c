...
int IOT_ForcedCH(qentry_t *pReq) 
{
		...
		{
				write(serialFD, "AT#CHFIX?\r" , strlen("AT#CHFIX?\r"));
				while (STOP==FALSE) {
				...
				char *ena = req->getstrf(req, false, "ena");
				char *dl_uarfcn = req->getstrf(req, false, "dl_uarfcn");
				
				char command[100];
				if (strcmp(ena, "0") == 0) {
						sprintf(command, "AT#CHFIX=%s\r", ena);
				} else {
						sprintf(command, "AT#CHFIX=%s,%s\r", ena, dl_uarfcn);
				}
