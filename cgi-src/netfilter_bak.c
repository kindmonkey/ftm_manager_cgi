...
int	FTMC_Netfilter(qentry_t *pReq)
{
    ...
    else if (strcmp(lpszCmd, "set") == 0)
    {
		...
		while(1)
		{
			...
			{
				if (strcmp("icmp", lpszProto) == 0)
				{
					fprintf(fp, "-p %s\t", lpszProto);
				}
				else
				{
					fprintf(fp, "-p %s -m %s\t", lpszProto, lpszProto);
				}
			}

			if (strcmp("echo-request", lpszDPort) == 0)
			{
				fprintf(fp, "--icmp-type %s\t", lpszDPort);
			}
			else if (strcmp("0", lpszDPort) != 0)
			{
				fprintf(fp, "--dport %s\t", lpszDPort);
			}
		...
		fprintf(fp, "-A FORWARD -i eth1 -j ACCEPT\n");
		fprintf(fp, "-A FORWARD -o eth1 -j ACCEPT\n");
		...
		fp = popen("service iptables restart", "r");
		if (fp != NULL)
		{
			pclose(fp);	
